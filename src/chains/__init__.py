...
def web_search(query: str, max_results: int = 5) -> str:
    ...
    try:
        if tv_search:  # Use Tavily if available
            ...
            results = search.run(f"{query} site:.edu OR site:.gov OR site:.org -site:twitter.com")
            return results[:max_results*1000]  # Rough character limit
    except Exception as e:
...
def get_all_tools() -> list:
    ...
    if tv_search:  # Add Tavily as separate tool if initialized
